...
def main(args):
    ...
    with open(analysis_path, 'w') as f:
        ...
        fewshot_impact = results_df.groupby('aggregate_fewshot')['accuracy'].mean().sort_values(ascending=False)
        ...
        if 'mse_overall' in results_df.columns and not results_df['mse_overall'].isna().all():
            ...
            mse_transcript_impact = results_df.groupby('transcript_type')['mse_overall'].mean().sort_values()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate audio dataset with pairwise comparison.")
    parser.add_argument("--dataset_name", type=str, default="somos", help="Name of the dataset to evaluate.")
    parser.add_argument("--model", type=str, default="gpt-4o-audio-preview", help="Model to use for evaluation.")
    parser.add_argument("--n_samples", type=int, default=200, help="Maximum number of samples to evaluate.")
