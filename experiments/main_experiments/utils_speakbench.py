...
def get_prompt(prompt_type: str, instruction_path: str, audio1_path: str, audio2_path: str, dataset_name: str, n_shots: int = 0, 
        ...
        if two_turns:
            ...
            for i in range(n_shots):
                ...
                instruction_content = [
                    ...
                    {"type": "input_audio", "input_audio": {"data": instruction, "format": "wav"}},
                ]
                if transcript_type != "none":
                    instruction_transcript = example.get("instruction_transcript")
                    instruction_content.append({"type": "text", "text": f"Here is the transcript for instruction: \"{instruction_transcript}\""})
                messages.append({
                    "role": "user",
            ...
            for i in range(n_shots):
                ...
                instruction_transcript = example.get("instruction_transcript")
                if transcript_type != "none":
                    content.append({"type": "text", "text": f"Transcript for instruction: \"{instruction_transcript}\""})
    ...
    if two_turns:
        ...
        if transcript_type == "groundtruth" or transcript_type == "asr":
            ...
            if transcript_instruction:
                instruction_content.append({"type": "text", "text": f"Transcript for this instruction: \"{transcript_instruction}\""})
        messages.append({
            "role": "user",
            "content": instruction_content
        })
        ...
        if transcript_type == "groundtruth" or transcript_type == "asr":
            ...
            user_content.append({"type": "text", "text": f"Transcript for this instruction: \"{transcript_instruction}\""})
...
def evaluate_prompt_strategy_speakbench(df: pd.DataFrame, prompt_type: str, model: str, dataset_name: str,
