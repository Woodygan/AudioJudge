...
def concatenate_audio_files(audio_paths: List[str], output_path: str, add_signals: bool = True, 
                        signal_folder: str = "signal_audios") -> str:
    """
    ...
    if add_signals:
        ...
        for i in range(len(audio_paths)):
            required_signals.append((f"Example {i+1}, audio:", f"example_{i+1}_audio.wav"))
    ...
    with wave.open(output_path, 'wb') as output_file:
        ...
        for i, audio_path in enumerate(audio_paths):
            ...
            if add_signals:
                ...
                silence_frames = b'\x00' * (int(0.3 * target_sample_rate) * sampwidth * nchannels)
    ...
    return output_path
def concatenate_audios_speakbench(audio_paths: List[str], output_path: str, n_shots: int) -> str:
    """
    ...
    with wave.open(output_path, 'wb') as output_file:
        ...
        for i in range(n_shots):
            ...
            long_silence = b'\x00' * (int(0.7 * target_sample_rate) * sampwidth * nchannels)
            output_file.writeframes(long_silence)
    
    return output_path
def get_prompt(prompt_type: str, instruction_path: str,  audio_path: str, dataset_name: str = "pronunciation", n_shots: int = 0, 
               transcript_type: str = "none", aggregate_fewshot: bool = False) -> List[Dict]:
    """
    ...
    if dataset_name == "tmhintq" or dataset_name == "somos" or dataset_name == "thaimos":
        if prompt_type != "no_cot":
            user_message = ("Please analyze the speech quality of this recording. "
        ...
        if prompt_type != "no_cot":
            user_message = ("Please analyze how well this recording follows the instruction. "
                "Respond ONLY in text and output valid JSON with keys 'reasoning' and 'score' (int from 1-5).")
        else:
    ...
    if dataset_name == "tmhintq" or dataset_name == "somos" or dataset_name == "thaimos":
        ...
        if n_shots > 0 and available_examples:
            ...
            if aggregate_fewshot:
                ...
                examples_content = [
                    {"type": "text", "text": "Here are some examples for reference (Note that their scores are mean option scores so are not necessarily integer, but you will need to give an integer score):"},
                ...
                for i in range(n_shots):
                    ...
                    if i == 0:
                        messages.append({
                            "role": "user",
                            "content": [{"type": "text", "text": "Here are some examples for reference (Note that their scores are mean option scores so are not necessarily integer, but you will need to give an integer score):"}]
                        })
                    user_content = [
                    ...
                    transcript = example.get("transcript")
                    if transcript_type != "none":
                        user_content.append({"type": "text", "text": f"Transcript for audio: \"{transcript}\""})
                    user_content.append({"type": "text", "text": f"Score: {example.get('score')}"})
        ...
        if n_shots > 0 and available_examples:
            ...
            if aggregate_fewshot:
                ...
                concat_examples_path = os.path.join("temp_audio", f"concat_examples_{time.time()}.wav")
                concatenate_audios_speakbench(all_example_audio_paths, concat_examples_path, n_shots)
                ...
                for i in range(n_shots):
                    ...
                    example_text += f"Example {i+1}:\n"
                    if transcript_type != "none":
                        example_text += f"- Instruction: \"{example['instruction_text']}\"\n"
                ...
                for i in range(n_shots):
                    ...
                    if i == 0:
                        messages.append({
                            "role": "user",
                            "content": [{"type": "text", "text": "Here are some examples for reference:"}]
                        })
                    ...
                    if transcript_type != "none":
                        content.append({"type": "text", "text": f"Instruction text: \"{instruction_text}\""})
                    content.extend([{"type": "text", "text": "Here is the audio response:"},
                        {"type": "input_audio", "input_audio": {"data": audio_encoded, "format": "wav"}}]),
                    if example_score is not None:
                        content.append({"type": "text", "text": f"Score: {example_score}"})
                    else:
                        raise ValueError(f"Score not available for example {i+1}")
        ...
        if transcript_type == "groundtruth":
            instruction_transcript = get_transcript(instruction_path, transcript_type, dataset_name)
            if instruction_transcript:
                user_content.append({"type": "text", "text": f"Instruction text: \"{instruction_transcript}\""})
        elif transcript_type == "asr":
...
def get_prompt_gemini(prompt_type: str, instruction_path: str, audio_path: str, dataset_name: str = "pronunciation", n_shots: int = 0, 
               transcript_type: str = "none", aggregate_fewshot: bool = False) -> List:
    """
    ...
    if n_shots > 0 and available_examples:
        ...
        if aggregate_fewshot:
            ...
            messages.append("Here are some examples for reference (Note that their scores are mean option scores so are not necessarily integer, but you will need to give an integer score):")
            messages.append({
                "mime_type": "audio/wav",
            ...
            messages.append("I'll apply this understanding to analyze the new audio clip you provide.")
...
def evaluate_prompt_strategy_pointwise(df: pd.DataFrame, prompt_type: str, model: str, dataset_name: str,
                             n_samples: int = 100, n_shots: int = 0, result_dir: str = "results_pointwise", 
    ...
    try:
        ...
        for _, row in tqdm(sample_df.iterrows(), total=len(sample_df), 
            ...
            if prediction_json1 is None or prediction_json2 is None:
                print(f"Failed to extract JSON from response for {audio1_path} or {audio2_path}")
                continue
            ...
            if score1 is None or score2 is None:
                print(f"No score field in extracted JSON for {audio1_path} or {audio2_path}")
                continue
            ...
            if dataset_name == "somos" or dataset_name == "thaimos" or dataset_name == "tmhintq":
                if prediction == "tie":
        ...
        if len(model_scores1) > 0 and len(gt_scores1) > 0:
            ...
            mse_overall = np.mean([(m - g)**2 for m, g in zip(all_model_scores, all_gt_scores)])
