...
from utils import get_model_response, encode_audio_file, extract_json_from_response, load_dataset
...
def evaluate_model_comparisons(df: pd.DataFrame, prompt_type: str, model: str, dataset_name: str,
    ...
    try:
        ...
        model_columns = [col for col in sample_df.columns 
        ...
        for _, row in tqdm(sample_df.iterrows(), total=len(sample_df), 
            ...
            if gpt4o_audio_path is None:
                print(f"Missing gpt4o-audio.wav for datapoint {row.get('index', 'unknown')}")
                continue
            ...
            for model_col in model_columns:
                ...
                if model_audio_path is None:
                    raise ValueError(f"Missing audio for model {model_col} in datapoint {index}")
                ...
                if prediction_json_pos1 is None or prediction_json_pos2 is None:
                    print(f"Failed to extract JSON from response for {model_col} vs gpt4o-audio")
                    continue
                ...
                if prediction_pos1 is None or prediction_pos2 is None:
                    print(f"No prediction field in extracted JSON for {model_col} vs gpt4o-audio")
                    continue
        ...
        print(f"Configuration: {prompt_type} judge, {n_shots} shots, {transcript_type} transcript")
        print("\nWin Rates vs gpt4o-audio (average of both positions):")
        
        # Sort by average win rate for display
        for model, metrics in sorted(final_results.items(), key=lambda x: x[1]['average_win_rate'], reverse=True):
            print(f"  {model}: {metrics['average_win_rate']:.2%} win rate ({metrics['tie_rate']:.2%} tie rate)")
