...
def load_detailed_csv_files(file_paths: List[str]):
    ...
    for file_path in file_paths:
        try:
            ...
            required_columns = [
                'instruction_id', 'model_name', 
                'position1_prediction', 'position2_prediction'
            ]
    ...
    print(f"Loaded {len(dataframes)} CSV files with {len(all_ids)} unique data points across {len(all_models)} models.")
...
def calculate_model_metrics(ensemble_df):
    ...
    metrics_df = metrics_df.sort_values('effective_win_rate', ascending=False).reset_index(drop=True)
...
def print_results_summary(metrics_df):
    ...
    print("-" * 80)
    print(f"{'Model':<30} | {'Eff Win Rate':<12} | {'Avg Win Rate':<12} | {'Avg Tie Rate':<12}")
    print("-" * 80)
    ...
    for _, row in metrics_df.iterrows():
        ...
        print(f"{model_name:<30} | {effective_win_rate:>10.2%}  | {avg_win_rate:>10.2%}  | {avg_tie_rate:>10.2%}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Ensemble detailed evaluation results from multiple CSV files.')
    parser.add_argument('--files', nargs='+', required=True, help='List of detailed CSV files to ensemble')
