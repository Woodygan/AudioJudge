...
from api_cache import api_cache, clear_cache, clear_none_cache # Import the caching decorator
...
def main(args):
    ...
    result_dir = os.path.join("results", args.dataset_name, args.model)  # Directory to save results 
    ...
    n_shots_list = [0]  # Including 0 (no examples)
    ...
    if "speakbench" in dataset or "chatbotarena" in dataset:
        for prompt_type in PROMPT_TYPES:
            ...
            for transcript_type in TRANSCRIPT_TYPES:
                ...
                for two_turns in TWO_TURNS_CONFIGS:
                    if two_turns:
                        if "gemini" in model:
                            raise ValueError("Gemini models are not supported for two_turn setting. Please use a compatible model.")
                        if transcript_type != "none":
                            continue
                        # When two_turns is True, other configs don't matter
                        config_desc = (f"{n_shots} shots, {transcript_type} transcript, "
                                    f"two_turns")
                        ...
                        for fewshot_config_name, fewshot_params in FEWSHOT_CONFIGS.items():
                            for concat_test in TEST_CONFIGS:
                                if transcript_type != "none" and (fewshot_config_name != "aggregate" or not concat_test):
                                    #continue
                                    pass
        ...
        if "gemini" in model:
            raise ValueError("Gemini models are not supported for this dataset. Please use a compatible model.")
        for prompt_type in PROMPT_TYPES:
            print(f"\nEvaluating prompt strategy: {prompt_type}")
    ...
    results_df.to_csv(f"{combined_dir}/all_results_{model.replace('-', '_')}.csv", index=False)
    ...
    with open(analysis_path, 'w') as f:
        ...
        comparison = results_df.sort_values('accuracy', ascending=False).to_string(index=False)
        print(comparison)
        ...
        if 'two_turns' in best_strategy and best_strategy['two_turns']:
            ...
            if 'aggregate_fewshot' in best_strategy and best_strategy['aggregate_fewshot']:
        ...
        transcript_impact = results_df.groupby('transcript_type')['accuracy'].mean().sort_values(ascending=False)
        ...
        if 'two_turns' in results_df.columns:
            turns_impact = results_df.groupby('two_turns')['accuracy'].mean().sort_values(ascending=False)
            turns_header = "\nTwo Turns Impact:"
            print(turns_header)
            f.write(f"{turns_header}\n")
            
            for two_turns, acc in turns_impact.items():
                turns_line = f"  {'Two Turns' if two_turns else 'Single Turn'}: {acc:.4f}"
                print(turns_line)
        ...
        if 'aggregate_fewshot' in results_df.columns:
            ...
            fewshot_impact = fewshot_df.groupby('fewshot_config')['accuracy'].mean().sort_values(ascending=False)
            ...
            fewshot_impact = results_df.groupby('concat_fewshot')['accuracy'].mean().sort_values(ascending=False)
            fewshot_header = "\nFew-shot Configuration Impact:"
            print(fewshot_header)
            f.write(f"{fewshot_header}\n")
        ...
        test_impact = results_df.groupby('concat_test')['accuracy'].mean().sort_values(ascending=False)
        test_header = "\nTest Configuration Impact:"
        print(test_header)
        f.write(f"{test_header}\n")
        
        for concat_test, acc in test_impact.items():
            test_line = f"  {'Concatenated' if concat_test else 'Separate'} Test: {acc:.4f}"
            print(test_line)
            print(test_line)
            f.write(f"{test_line}\n")
        
        # Impact of shots
        shots_impact = results_df.groupby('n_shots')['accuracy'].mean().sort_values(ascending=False)
        ...
        prompt_impact = results_df.groupby('prompt_type')['accuracy'].mean().sort_values(ascending=False)
        prompt_header = "\nPrompt Type Impact:"
        print(prompt_header)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate audio pronunciation dataset with various prompt strategies.")
    parser.add_argument("--dataset_name", type=str, default = "pronunciation", help="Name of the dataset to evaluate.")
    parser.add_argument("--model", type=str, default = "gpt-4o-audio-preview", help="Model to use for evaluation.")
