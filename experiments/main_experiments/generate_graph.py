...
def generate_comprehensive_heatmap(results_df, result_dir, model_name):
    ...
    if 'two_turns' in results_df.columns:
        ...
        if 'aggregate_fewshot' in results_df.columns:
            results_df['config_type'] = results_df.apply(
                lambda row: "Two Turns" if row['two_turns'] else 
                ("Aggregate" if row['aggregate_fewshot'] else 
                 ("Concat" if row['concat_fewshot'] else "Separate")), axis=1
            )
        else:
            results_df['config_type'] = results_df.apply(
                lambda row: "Two Turns" if row['two_turns'] else 
        ...
        if 'aggregate_fewshot' in results_df.columns:
            results_df['config_type'] = results_df.apply(
                lambda row: "Aggregate" if row['aggregate_fewshot'] else 
                ("Concat" if row['concat_fewshot'] else "Separate"), axis=1
            )
        else:
            results_df['config_type'] = results_df.apply(
    ...
    for transcript_type in results_df['transcript_type'].unique():
        ...
        for n_shots in sorted(results_df['n_shots'].unique()):
            ...
            if "Two Turns" in results_df['config_type'].unique():
                two_turns_data = results_df[(results_df['transcript_type'] == transcript_type) & 
                ...
                if len(two_turns_data) > 0:
                    ...
                    placeholder_data = results_df[(results_df['transcript_type'] == transcript_type) & 
            ...
            for config_type in [ct for ct in results_df['config_type'].unique() if ct != "Two Turns"]:
            ...
            config_vals = [v for k, v in row_data.items() if k != 'n_shots' and isinstance(v, (int, float))]
            if config_vals:
        ...
        for col in pivot_df.columns:
            numeric_values.extend([v for v in pivot_df[col] if isinstance(v, (int, float))])
        ...
        transcript_name = transcript_type.capitalize() if transcript_type != "none" else "No Transcript"
        plt.title(f'Configuration Comparison\n{transcript_name}\n{model_name}', 
        ...
        plt.savefig(os.path.join(plots_dir, f'{filename_base}.pdf'), bbox_inches='tight')
        plt.close()
    for transcript_type in results_df['transcript_type'].unique():
        for config_type in results_df['config_type'].unique():
            ...
            if config_type == "Two Turns":
                ...
                # Configure the plot
                plt.title(f'{model_name}: Pronunciation Comparison Accuracy\n{transcript_name} - {config_name}', 
                         fontsize=18, fontweight='bold')
    ...
    for config_type in results_df['config_type'].unique():
        if config_type == "Two Turns":
            ...
            for transcript_type in results_df['transcript_type'].unique():
                ...
                if len(filtered_df) > 0:
                    transcript_comparison[transcript_type] = filtered_df['accuracy'].mean()
            ...
            # Sort by accuracy
            sorted_transcripts = sorted(transcript_comparison.items(), key=lambda x: x[1], reverse=True)
            x = [t.capitalize() if t != "none" else "No Transcript" for t, _ in sorted_transcripts]
            ...
            plt.title(f'Transcript Type Comparison\n{config_type} Configuration\n{model_name}', 
    ...
    for config_type in [ct for ct in results_df['config_type'].unique() if ct != "Two Turns"]:
        for concat_test in results_df['concat_test'].unique():
            for transcript_type in results_df['transcript_type'].unique():
                ...
                if len(filtered_df) > 0:
                    transcript_name = transcript_type.capitalize() if transcript_type != "none" else "No Transcript"
                    config_name = f"{config_type}_{('Concat' if concat_test else 'Separate')}"
                    avg_accuracy = filtered_df['accuracy'].mean()
                    config_matrix.append({
                        'transcript': transcript_name,
                        'config': config_name,
    ...
    if len(config_matrix) > 0:
        ...
        # Configure the plot
        plt.title(f'{model_name}: Comprehensive Configuration Comparison\nTranscript Type vs. Configuration', 
        ...
        print(f"Comprehensive configuration comparison heatmap saved to {plots_dir}/{filename_base}.png")
