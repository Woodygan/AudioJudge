...
def main(args):
    ...
    prompt_types = args.prompt_types if hasattr(args, 'prompt_types') else ["standard_cot"]
    ...
    results_df.to_csv(f"{combined_dir}/model_comparison_results_{timestamp}.csv", index=False)
    ...
    if not results_df.empty:
        ...
        for _, row in model_summary.sort_values(('average_win_rate', 'mean'), ascending=False).iterrows():
            ...
            print(f"  {model}: {win_rate:.2%} ± {win_std:.2%} win rate ({tie_rate:.2%} tie rate) from {count} configurations")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate model comparisons against gpt4o-audio")
    parser.add_argument("--dataset_name", type=str, default="speakbench", help="Name of the dataset")
    parser.add_argument("--model", type=str, default = "gpt-4o-audio-preview", help="Model to use as judge")
    parser.add_argument("--n_samples", type=int, default=100, help="Number of samples to evaluate")
    parser.add_argument("--n_shots", type=int, default=None, help="Number of few-shot examples to use (if None, will try 0, 2, 4)")
    parser.add_argument("--prompt_types", nargs="+", default=["standard_cot"], 
