...
def system_level_analysis_csv(path_ab, path_ba):
    ...
    all_models_a = set(data_ab_matched['model_a'].tolist() + data_ba_matched['model_a'].tolist())
    all_models_b = set(data_ab_matched['model_b'].tolist() + data_ba_matched['model_b'].tolist())
    ...
    for system, pred in system_preds.items():
        ...
        assert len(pred) == len(gts), f"Mismatch for {system}: pred={len(pred)}, gts={len(gts)}"
    ...
    for i, system in enumerate(system_names):
        print(f"{system}: GT={system_level_gts[i]:.3f}, Pred={system_level_pred[i]:.3f}, Pairs={count_pair_each_systems[i]}")
...
def analyze_correlation_and_plot(list_a, list_b, system_names=None):
    ...
    if system_names:
        for i, name in enumerate(system_names):
            plt.annotate(name, (list_a[i], list_b[i]), 
    ...
    plt.title(f'System-Level Correlation\nSpearman: {spearman_corr:.3f}', fontsize=14, fontweight='bold')
