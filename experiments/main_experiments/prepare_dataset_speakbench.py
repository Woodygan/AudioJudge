...
def prepare_speakbench_dataset(huggingface_repo, output_path, speakbench508_json_path="datasets/speakbench508_dataset.json", audio_dir="speakbench_audio"):
    """
    ...
    print(f"Loaded {len(instruction_id_to_path)} instruction audio paths from speakbench508 dataset")
    ...
    for _, row in tqdm(hf_df.iterrows(), total=len(hf_df)):
        ...
        if instruction_id and instruction_id in instruction_id_to_path:
            ...
            if os.path.exists(source_instruction_path):
                ...
                try:
                    ...
                    raise RuntimeError(f"Failed to copy instruction audio for index {index}, instruction_id {instruction_id}: {str(e)}")
            else:
                raise FileNotFoundError(f"Instruction audio file not found for index {index}, instruction_id {instruction_id}: {source_instruction_path}")
        ...
        for column_name, column_value in row.items():
            ...
            try:
                ...
                if isinstance(column_value, dict) and 'array' in column_value and 'sampling_rate' in column_value:
                    # HuggingFace audio format with array and sampling rate
                    sf.write(file_path, column_value['array'], column_value['sampling_rate'])
                    ...
                    print(f"Skipping column '{column_name}' for index {index} (not an audio format)")
                    continue
                ...
                print(f"Error saving audio for column '{column_name}', index {index}: {str(e)}")
                continue
