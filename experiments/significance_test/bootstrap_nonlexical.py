...
def load_data(file_path, id_column, result_column):
    ...
    try:
        ...
        if id_column not in df.columns or result_column not in df.columns:
            raise ValueError(f"Columns {id_column} or {result_column} not found in {file_path}")
...
def filter_common_datapoints(df_a, df_b, id_column):
    ...
    print(f"Filtered out {len(df_a) - len(common_ids)} from A and {len(df_b) - len(common_ids)} from B")
...
def bootstrap_difference_independent_two_sided(data_a, data_b, n_samples=10000, confidence_level=0.95):
    """
    ...
    is_significant = (lower_bound > 0 and upper_bound > 0) or (lower_bound < 0 and upper_bound < 0)
    
    return observed_diff, (lower_bound, upper_bound), is_significant, bootstrap_diffs

def bootstrap_difference_paired_one_sided(data_a, data_b, n_samples=10000, confidence_level=0.95):
    """
...
def main():
    ...
    parser.add_argument('-a', '--file-a', required=True,
                        help='Path to first CSV file (method A)')
    parser.add_argument('-b', '--file-b', required=True,
                        help='Path to second CSV file (method B)')
    parser.add_argument('--id-column', default='audio1_path',
                        help='Name of the ID column')
    parser.add_argument('--result-column', default='correct',
                        help='Name of the result column')
    parser.add_argument('--method', default='paired_one_sided',
    ...
    if args.method == 'independent_two_sided':
        ...
        observed_diff, ci, is_significant, bootstrap_diffs = bootstrap_difference_independent_two_sided(
            data_a, data_b, n_samples=N_SAMPLES, confidence_level=CONFIDENCE_LEVEL
        )
        
        # Calculate p-value (two-tailed)
        p_value = min(
            np.mean(bootstrap_diffs <= 0) * 2,
            np.mean(bootstrap_diffs >= 0) * 2
        )
        ...
        if is_significant:
            print(f"The change is statistically significant ({CONFIDENCE_LEVEL*100}% CI does not include zero)")
        else:
            print(f"The change is not statistically significant ({CONFIDENCE_LEVEL*100}% CI includes zero)")

    elif args.method == 'paired_one_sided':
        # Perform bootstrap analysis --- paired, one-sided
        print(f"\nPerforming bootstrap analysis with {N_SAMPLES} samples...")
        observed_diff, p_value, is_significant, bootstrap_diffs = bootstrap_difference_paired_one_sided(
            data_a, data_b, n_samples=N_SAMPLES, confidence_level=CONFIDENCE_LEVEL
        )
        # Display results
        print("\nResults:")
        ...
        if is_significant:
            print(f"Method B is significantly better than Method A (p < {1-CONFIDENCE_LEVEL:.2f})")
        else:
            print(f"No significant difference found (p >= {1-CONFIDENCE_LEVEL:.2f})")
    else:
        print(f"Error: Unknown method '{args.method}'. Supported methods are 'independent_two_sided' and 'paired_one_sided'.")
