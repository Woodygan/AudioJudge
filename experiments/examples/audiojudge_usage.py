...
def get_user_prompt(dataset_name: str) -> str:
    ...
    if dataset_name == "pronunciation":
        user_prompt = ("Please analyze these two recordings strictly for pronunciation details (phonemes, syllables, stress, emphasis). "
        ...
        user_prompt = ("Please analyze if these two recordings are from the same speaker. "
            "Respond ONLY in text and output valid JSON with keys 'reasoning' and 'match' (boolean).")
    elif dataset_name == "speed":
        user_prompt = ("Please analyze which of the two recordings has faster speech. "
        ...
        user_prompt = ("Please analyze which of the two recordings is better (has better speech quality). "
            "Respond ONLY in text and output valid JSON with keys 'reasoning' and 'label' (string, either '1' or '2').")
def main(args):
    ...
    for _, row in tqdm(sample_df.iterrows(), total=len(sample_df), desc="Processing samples"):
        try:
            ...
            if not response["success"]:
                print(f"Failed to process {audio1_path}, {audio2_path}: {response.get('error', 'Unknown error')}")
                failed_samples += 1
                continue
            ...
            if prediction_json is None:
                print(f"Failed to extract JSON from response for {audio1_path}, {audio2_path}")
                failed_samples += 1
                continue
            ...
            if prediction is None:
                print(f"No '{prediction_key}' field in extracted JSON for {audio1_path}, {audio2_path}")
                failed_samples += 1
                continue
