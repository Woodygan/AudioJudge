...
def analyze_positional_bias(csv_file_path):
    ...
    both_predict_1 = (df_clean['position1_prediction'] == '1') & (df_clean['position2_prediction'] == '1')
    both_predict_2 = (df_clean['position1_prediction'] == '2') & (df_clean['position2_prediction'] == '2')
    consistent_predictionsAB = (df_clean['position1_prediction'] == '1') & (df_clean['position2_prediction'] == '2')
    consistent_predictionsBA = (df_clean['position1_prediction'] == '2') & (df_clean['position2_prediction'] == '1')
    # Count bias cases
    ...
    total_cases= both_1_count + both_2_count + consistent_AB_count + consistent_BA_count
    print(f"Bias percentage where both predictions are '1': {both_1_count/len(df_clean) * 100:.2f}%)")
    print(f"Bias percentage where both predictions are '2': {both_2_count/len(df_clean) * 100:.2f}%)")
