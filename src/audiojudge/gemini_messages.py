...
def get_gemini_messages_with_instruction(instruction_path: str,
    ...
    if concatenation_method == "no_concatenation":
        ...
        if examples:
            _add_triplet_concatenated_examples_gemini(messages, examples, openai_client, signal_folder)
        _add_separate_test_audio_with_instruction_gemini(messages, instruction_path, audio1_path, audio2_path, user_prompt)
...
def _add_triplet_concatenated_examples_gemini(messages: List, 
                                            examples: List[AudioExample], 
                                            openai_client,
                                            signal_folder: str):
    """Add examples with each triplet (instruction + audio1 + audio2) concatenated into one audio file for Gemini."""
    # Create temp directory if it doesn't exist
    os.makedirs("temp_audio", exist_ok=True)
